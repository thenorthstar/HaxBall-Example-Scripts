...
function chat_xpSending(player, message) { //!xp 32 5. Send 5 XPs to the player whose ID is 32.
    ...
    if (message.split(" ")[0] == commands[5]) {
        if(JSON.parse(localStorage.getItem("playerList"))[player.name].canSendXP == false){
                        ...
                        if(ID == player.id) {
                            ...
                            if (isNaN(xp)) {
                                ...
                                if (xp < toleranceObject.xpSending.least || toleranceObject.xpSending.most < xp) {
                                    ...
                                    if (xp > JSON.parse(localStorage.getItem("playerList"))[player.name].xp) {
                                        room.sendAnnouncement(`${messages.Chat.XPSending.Insufficient}`, player.id, colors.Chat.XPSending.Insufficient, fonts.Chat.XPSending.Insufficient, sounds.Chat.XPSending.Insufficient);
                                        return false;
                                    }
                                    else {
...
function getPreviousAccounts(name) {
    return playerListArray.filter(p => p.auth == JSON.parse(localStorage.getItem("playerList"))[name].auth || p.auth == JSON.parse(localStorage.getItem("playerList"))[name].conn);
}
