...
var room = HBInit({ roomName: "Mute Players (Delayed Version)", noPlayer: true, public: true, maxPlayers: 12 });

function findRelatedAccounts(auth, conn) {
    if (playerList.length == 0) return [];
    else return Object.values(playerList).filter(p => p.auth === auth);
}
...
function partitionRemainingTime(remainingTime) {
...
room.onPlayerChat = function (player, message) {
    ...
    if (isCommand(message) == true) {
                        ...
                        if (p.admin == true) {
                            ...
                            if (isNaN(time)) {
                                ...
                                if (time < muteRange.Min || muteRange.Max < time) {
                                    ...
                                    if (muteList.find(m => m.auth === playerList[p.name].auth || m.conn === playerList[p.name].conn)) {
                                        ...
                                        var pname = p.name;
                                        var pauth = playerList[p.name].auth;
                                        var pconn = playerList[p.name].conn;
                                        muteList.push({ name: pname, auth: pauth, conn: pconn, start: Date.now(), end: Date.now() + time * timeObject.Minutes, duration: time * timeObject.Minutes, timeoutFunction: -1 });
                                        var muteFunc = setTimeout(function () {
                                            if (muteList.findLast(m => m.auth == pauth)) {
                                                var index = muteList.findLastIndex(m => m.auth == pauth);
                                                if (index !== -1) muteList.splice(index, 1);
                                                room.sendAnnouncement(`${pname} ${messages.Unmute}`, null, colors.Unmute, fonts.Unmute, sounds.Unmute);
                                            }
                                        }, time * timeObject.Minutes);
                                        muteList.at(-1).timeoutFunction = muteFunc;
                                        return false;
                                    }
                                }
                            }
        ...
        if (player.admin == false && muteList.find(m => m.auth === playerList[player.name].auth) != undefined) {
            var remainingTime = muteList.findLast(m => m.auth === playerList[player.name].auth).end - Date.now();
            var timeParts = partitionRemainingTime(remainingTime);
            room.sendAnnouncement(`${messages.Mute.Still} (${message}) Remaining: ${timeParts.minutes == 1 ? "1 minute" : `${timeParts.minutes} minutes`} ${timeParts.seconds < 10 ? "0" : ""}${timeParts.seconds == 1 ? "1 second" : `${timeParts.seconds} seconds`}`, player.id, colors.Mute.AdminCanSee, fonts.Mute.AdminCanSee, sounds.Mute.AdminCanSee);
...
room.onPlayerJoin = function (player) {
    ...
    var relatedAccounts = findRelatedAccounts(player.auth, player.conn);

    if (relatedAccounts.length == 0) {
        playerList[player.name] = { name: player.name, auth: player.auth, conn: player.conn, lastLeaveDate: -Infinity, muted: { value: false, start: -1, end: -1, timeoutFunction: -1 } };
    }
    else {
        playerList[player.name].lastLeaveDate = relatedAccounts.at(-1).lastLeaveDate;
        if (muteList.find(m => m.auth == player.auth)) {
            var muteEntry = muteList.find(m => m.auth == player.auth);
            muteEntry.end += (Date.now() - playerList[player.name].lastLeaveDate);
            var remainingTime = muteEntry.end - Date.now();

            var pname = player.name;
            var pauth = player.auth;
            var pconn = player.conn;

            var timeParts = partitionRemainingTime(remainingTime);
            room.sendAnnouncement(`${messages.Mute.Still} Remaining: ${timeParts.minutes == 1 ? "1 minute" : `${timeParts.minutes} minutes`} ${timeParts.seconds < 10 ? "0" : ""}${timeParts.seconds == 1 ? "1 second" : `${timeParts.seconds} seconds`}`, player.id, colors.Mute.Still, fonts.Mute.Still, sounds.Mute.Still);
...
room.onPlayerLeave = function (player) {
    playerList[player.name].lastLeaveDate = Date.now();
    console.log(`${player.name} has left`);

    if (muteList.find(m => m.auth === playerList[player.name].auth)) {
        var muteEntry = muteList.find(m => m.auth === playerList[player.name].auth);
        var remainingTime = muteEntry.end - Date.now();
        var timeParts = partitionRemainingTime(remainingTime);
        room.sendAnnouncement(`Remaining Mute for ${player.name}: ${timeParts.minutes == 1 ? "1 minute" : `${timeParts.minutes} minutes`} ${timeParts.seconds < 10 ? "0" : ""}${timeParts.seconds == 1 ? "1 second" : `${timeParts.seconds} seconds`}`, null, colors.Mute.Still, fonts.Mute.Still, sounds.Mute.Still);
        clearTimeout(muteEntry.timeoutFunction);
    }
}
